...
type FieldDef = {
  ...
  required?: boolean;
  // Campos ocultos: no se muestran en el front pero su valor (de defaultValues)
  // se sigue enviando en el cuerpo de la petición a la API.
  hidden?: boolean;
};
...
function ApiDrawer({
  ...
  // Solo los campos visibles se renderizan en el formulario.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Se envían TODOS los campos (incluidos los ocultos: paciente, doctor y cita).
    fields.forEach((f) => {
      const raw = values[f.key] ?? "";
      if (f.hidden && raw === "") return; // omitir IDs opcionales vacíos
...
// Los IDs (paciente, doctor, cita) van marcados como `hidden`:
// no se muestran en el front, pero se envían a la API desde CONTEXT_IDS.
// ─────────────────────────────────────────────
...
// Valores por defecto con los IDs ocultos (paciente, doctor, cita).
const ID_DEFAULTS = {
...
// ─────────────────────────────────────────────
// TAB: DATOS GENERALES
// POST /api/datosmedicos  |  POST /api/patientalergias
// ─────────────────────────────────────────────
  ...
  const inner = (isMobile = false) => (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-3 pb-4 pt-3 space-y-0.5 shrink-0">
        ...
        <button className={`w-full flex items-center gap-3 rounded-lg text-red-500 hover:bg-red-50 transition-all duration-150 ${collapsed && !isMobile ? "px-0 py-3 justify-center" : "px-3 py-2.5"}`} title={collapsed && !isMobile ? "Cerrar sesión" : undefined}>
